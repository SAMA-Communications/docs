...
<Aside title="Info">This handler checks for banned words and rejects the message if any are found. Otherwise, it allows the message through.</Aside>
...
1. ### 👋 Auto-Greeting Detection
    ...
    const handler = async (message, user, accept, resolve, reject, fetch) => {
      ...
      if (greetings.some((g) => body.startsWith(g))) {
        return resolve({
          message: {
            body: `Hi ${user.first_name || "there"}! 👋 How can I help you today?`,
          },
        });
      }
...
6. ### ✨ Auto-format Messages
    ...
    const handler = async (message, user, accept, resolve, reject, fetch) => {
      const body = message.body;
      const transformed = body.replace(/:smile:/g, "😄").replace(/:fire:/g, "🔥");
